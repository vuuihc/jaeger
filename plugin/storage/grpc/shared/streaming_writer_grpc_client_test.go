...
func TestStreamClientWriteSpanStream(t *testing.T) {
	withStreamingWriterGRPCClient(func(r *grpcStreamWriterClientTest) {
		...
		r.streamingSpanWriter.On("WriteSpanStream", mock.Anything).Return(stream, nil)
...
func TestStreamClientClose(t *testing.T) {
	withStreamingWriterGRPCClient(func(r *grpcStreamWriterClientTest) {
		...
		r.spanWriter.On("Close", context.Background(), &storage_v1.CloseWriterRequest{}).Return(nil, status.Errorf(codes.DeadlineExceeded, ""))
		err = r.client.Close()
		assert.Error(t, err)
	})
}
