...
To support writing span via client stream (which can enlarge the throughput`), you must fill `StreamingSpanWriter` with the same plugin with `Store` field. Note that client stream may make the `save_by_svr` metric inaccurate, in which case you'll need to pay attention to the metrics provided by plugin.
